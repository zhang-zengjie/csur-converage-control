...
%         prev_published_dC_neighbor
    ...
    methods
        ...
        function move(obj, w)
            move@FixedWingBase(obj, obj.controlParam.V_CONST(obj.ID), w);
        end
        ...
        function [Vk, wOut] = compute_control_input(obj, report)
            ...
            [Vk, wOut] = obj.controller.compute(obj.AgentPose_3d, obj.voronoiComputer, obj.ID);
            assert(Vk >= 0);
...
%             epsSigmoid = 10;
%             calc_W = obj.controlParam.W_ORBIT + gamma * obj.controlParam.W_ORBIT * obj.controlParam.V_CONST(obj.ID) *sigmoid_func((obj.VMCoord_2d - obj.CVTCoord_2d)' * [cos(obj.AgentPose_3d(3)) ; sin(obj.AgentPose_3d(3))],epsSigmoid); 
%             
%             % Predict the next state            
%             predict_Pose_3d = zeros(3,1);
%             predict_Pose_VM = zeros(2,1);
%             predict_Pose_3d(1) = obj.AgentPose_3d(1) + obj.dt * (obj.controlParam.V_CONST(obj.ID) * cos(obj.AgentPose_3d(3)));
%             predict_Pose_3d(2) = obj.AgentPose_3d(2) + obj.dt * (obj.controlParam.V_CONST(obj.ID) * sin(obj.AgentPose_3d(3)));
%             predict_Pose_3d(3) = obj.AgentPose_3d(3) + obj.dt * calc_W;
%             predict_Pose_VM(1) = predict_Pose_3d(1) - (obj.controlParam.V_CONST(obj.ID)/obj.controlParam.W_ORBIT(obj.ID)) * sin(predict_Pose_3d(3)); 
%             predict_Pose_VM(2) = predict_Pose_3d(2) + (obj.controlParam.V_CONST(obj.ID)/obj.controlParam.W_ORBIT(obj.ID)) * cos(predict_Pose_3d(3)); 
%             
%             isValid = true;
...
%                 obj.w = obj.controlParam.W_ORBIT(obj.ID);
%             end
%         end 

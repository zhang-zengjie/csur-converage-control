...
classdef centralized_controller < handle
    ...
    methods
        ...
        function obj = centralized_controller(SIM_PARAM, REGION_CONFIG, CONTROL_PARAM)
            % Default empty variables
            obj.parameters.Q = CONTROL_PARAM.Q2x2;
            obj.parameters.gamma = CONTROL_PARAM.GAMMA;
            obj.parameters.eps = CONTROL_PARAM.EPS_SIGMOID;
            obj.nAgent = SIM_PARAM.N_AGENT;
            obj.boundariesVertexes = REGION_CONFIG.BOUNDARIES_VERTEXES;
            obj.boundariesCoeff = REGION_CONFIG.BOUNDARIES_COEFF;
            obj.xrange = max(obj.boundariesVertexes(:,1));
            obj.yrange = max(obj.boundariesVertexes(:,2));

            % Init list of controller
            obj.agentList = agent_dynamics.empty(obj.nAgent, 0);
            for i = 1 : obj.nAgent
                botID = i;
                obj.agentList(i) = agent_dynamics(SIM_PARAM.TIME_STEP);
                obj.agentList(i).begin(botID, obj.boundariesCoeff, SIM_PARAM.START_POSE(i, :), CONTROL_PARAM.V_CONST(i), CONTROL_PARAM.W_ORBIT(i));
            end
        end   
        
        
        function [Info, poseCVT_2D, v, c] = updateCoverage(obj, newPoseVM_3D)
            format long;
            ...
            for thisAgent = 1: Info.Common.nAgent
                ...
                for j = 1: size(obj.boundariesCoeff)
                    ...
                    sumHj = sumHj + 1/hj;
                    sum_aj_HjSquared = sum_aj_HjSquared + [obj.boundariesCoeff(j,1); obj.boundariesCoeff(j,2)] / hj^2 ; 
                end
                ...
                Vk = (zk - Ck)' * Q * (zk - Ck) * sumHj/ 2;
                dVkdzk = (eye(2) - dCi_dzi')*Q_zDiff_div_hj + sum_aj_HjSquared * (zk - Ck)' * Q * (zk - Ck)/ 2;
        ...
        function [Info, ControlInput] = controlCentralize(obj, Info)
            ...
            for thisAgent = 1 : obj.nAgent
                ...
                ControlInput(thisAgent) = w0 + obj.parameters.gamma * w0 * sigmoid_func(dVkx * cosTheta + dVky * sinTheta, obj.parameters.eps); 
        ...
        function [Info, BLF, loggedTopics, v, c] = loop(obj)
            ...
            [AgentReport, poseCVT_2D, v, c] = obj.updateCoverage(newPoseVM);

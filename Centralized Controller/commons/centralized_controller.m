...
classdef centralized_controller < handle
    ...
    methods
        ...
        function obj = centralized_controller(nAgent, simConfig, param)
            % Default empty variables
            obj.parameters.Q = param.Q;
            obj.parameters.gamma = param.gamma;
            obj.parameters.eps = simConfig.epsSigmoid;
            obj.nAgent = nAgent;
            obj.boundariesVertexes = simConfig.bndVertexes;
            obj.boundariesCoeff = simConfig.bndCoeff;
            obj.xrange = max(simConfig.bndVertexes(:,1));
            obj.yrange = max(simConfig.bndVertexes(:,2));

            % Init list of controller
            obj.agentList = agent_dynamics.empty(obj.nAgent, 0);
            for i = 1 : obj.nAgent
                botID = i;
                obj.agentList(i) = agent_dynamics(simConfig.dt);
                obj.agentList(i).begin(botID, obj.boundariesCoeff, simConfig.agentInitPose(i, :), simConfig.agentVConst(i), simConfig.agentWOrbit(i));
            end
        end   
